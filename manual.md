...
#### Cursor IDE를 실행한 뒤 작업할 프로젝트 디렉토리를 연다. 해당 디렉토리는 Git 저장소 루트이거나 실제 개발이 이루어질 최상위 폴더여야 한다.
...
#### AI Agent가 프로젝트를 정확히 이해하도록 모델 선택 후 Agent 모드에서 프로젝트 규칙을 정의한다.
...
### 정의된 프로젝트 규칙을 기반으로 AI에게 개발 단계(Task)와 마일스톤 생성을 요청한다.
...
### 확정된 개발 계획을 기반으로 AI Agent에게 실제 개발 작업을 위임한다. Plan 상태에서 Build를 클릭하거나 Agent 모드로 전환해 플랜에 맞춰 로컬에서 작업할것을 요청한다.
